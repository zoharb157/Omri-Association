...
def create_comparison_chart(expenses_df: pd.DataFrame, donations_df: pd.DataFrame) -> None:
    ...
    try:
        ...
        try:
            ...
            expenses_df_copy['תאריך'] = pd.to_datetime(expenses_df_copy['תאריך'], errors='coerce', infer_datetime_format=True)
            valid_expenses = expenses_df_copy.dropna(subset=['תאריך'])

            donations_df_copy = donations_df.copy()
            donations_df_copy['תאריך'] = pd.to_datetime(donations_df_copy['תאריך'], errors='coerce', infer_datetime_format=True)
