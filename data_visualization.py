...
def create_comparison_chart(expenses_df: pd.DataFrame, donations_df: pd.DataFrame) -> None:
    ...
    try:
        ...
        if (
            "תאריך" not in expenses_df.columns
            or "שקלים" not in expenses_df.columns
            or "תאריך" not in donations_df.columns
            or "שקלים" not in donations_df.columns
        ):
        ...
        try:
            ...
            monthly_expenses = (
                valid_expenses.groupby(valid_expenses["תאריך"].dt.strftime("%Y-%m"))["שקלים"]
                .sum()
                .reset_index()
            )
            monthly_donations = (
                valid_donations.groupby(valid_donations["תאריך"].dt.strftime("%Y-%m"))["שקלים"]
                .sum()
                .reset_index()
            )
        ...
        st.plotly_chart(
            fig, use_container_width=True, key=f"comparison_chart_{uuid.uuid4().hex[:8]}"
        )
...
def create_monthly_trends(expenses_df: pd.DataFrame, donations_df: pd.DataFrame):
    ...
    try:
        ...
        if (
            "תאריך" not in expenses_df.columns
            or expenses_amount_col not in expenses_df.columns
            or "תאריך" not in donations_df.columns
            or donations_amount_col not in donations_df.columns
        ):
...
def create_budget_distribution_chart(df: pd.DataFrame):
    ...
    if not name_col or not amount_col:
        logging.error(
            f"Invalid or missing data - required columns not found. Available: {list(df.columns)}"
        )
        return None
...
def create_widows_support_chart(df: pd.DataFrame):
    ...
    if (
        ...
        or "סכום חודשי" not in df.columns
        or df.empty
    ):
        logging.error(
            f"Invalid or missing data - name_col: {name_col}, available columns: {list(df.columns)}"
        )
        return None
    try:
        # Convert amount column to numeric, handling mixed data types
        ...
        # Update traces
        fig.update_traces(hovertemplate="שם: %{x}<br>סכום חודשי: ₪%{y:,.0f}<extra></extra>")
...
def create_monthly_budget_chart(budget_status: dict) -> None:
    ...
    try:
        ...
        fig.update_layout(
            ...
            xaxis_title="חודש",
            yaxis_title="סכום (₪)",
            yaxis2=dict(title="יתרה (₪)", overlaying="y", side="right"),
            barmode="group",
            hovermode="x unified",
            showlegend=True,
            legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="center", x=0.5),
