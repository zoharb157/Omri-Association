...
def load_data():
    ...
    try:
        ...
        if len(exp_values) >= 3:
            exp = pd.DataFrame(
                exp_values[2:], columns=exp_values[1]
            )  # Skip title row, use row 1 as headers
            exp = exp.rename(columns={"NaT": "תאריך", "שם לקוח": "שם", "סכום": "שקלים"})
        ...
        if len(don_values) >= 3:
            ...
            don = don.rename(columns={"NaT": "תאריך", "שם התורם": "שם", "סכום": "שקלים"})
        ...
        if len(inv_values) >= 3:
            ...
            inv = inv.rename(columns={"NaT": "תאריך", "שם התורם": "שם", "סכום": "שקלים"})
        ...
        if len(alman_values) >= 2:
            alman = pd.DataFrame(
                alman_values[1:], columns=alman_values[0]
...
def clean_widows_data(alman):
    ...
    if "סכום חודשי" in alman.columns:
        ...
        alman["סכום חודשי"] = (
            alman["סכום חודשי"].str.replace("₪", "").str.replace(",", "").str.strip()
        )

        # Handle various empty/zero cases
        alman["סכום חודשי"] = alman["סכום חודשי"].replace(
            ["", "0", "0.0", "nan", "None", "null"], pd.NA
        )
        ...
        print(
            f"Cleaned monthly amounts: {alman['סכום חודשי'].value_counts(dropna=False).to_dict()}"
        )
