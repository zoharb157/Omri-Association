...
class TestDataIntegrity(unittest.TestCase):
    ...
    def test_required_columns_exist(self):
        ...
        for col in required_expenses_columns:
            self.assertIn(col, expenses_df.columns, f"Required column '{col}' missing from expenses DataFrame")
        ...
        for col in required_donations_columns:
            self.assertIn(col, donations_df.columns, f"Required column '{col}' missing from donations DataFrame")
        ...
        for col in required_almanot_columns:
            self.assertIn(col, almanot_df.columns, f"Required column '{col}' missing from almanot DataFrame")
    ...
    def test_data_types_validation(self):
        ...
        for col in ["שם", "תורם"]:
            self.assertTrue(pd.api.types.is_string_dtype(df[col]), f"Column '{col}' should be string")
    ...
    def test_date_validation(self):
        ...
        for date in df["תאריך"]:
            ...
            self.assertLessEqual(date.year, current_year + 1, "Date should not be too far in the future")
    ...
    def test_hebrew_text_validation(self):
        ...
        for _, row in df.iterrows():
            self.assertTrue(any('\u0590' <= char <= '\u05FF' for char in row["שם"]), "Name should contain Hebrew characters")
            self.assertTrue(any('\u0590' <= char <= '\u05FF' for char in row["תורם"]), "Donor should contain Hebrew characters")
    ...
    def test_numeric_range_validation(self):
        ...
        self.assertGreaterEqual(df["מספר ילדים"].min(), 0, "Number of children should not be negative")
        self.assertLessEqual(df["מספר ילדים"].max(), 50, "Number of children should not be unreasonably high")

        self.assertGreaterEqual(df["סכום חודשי"].min(), 0, "Monthly amount should not be negative")
        self.assertLessEqual(df["סכום חודשי"].max(), 1000000, "Monthly amount should not be unreasonably high")
...
class TestBusinessLogicValidation(unittest.TestCase):
    ...
    def test_donor_widow_consistency(self):
        ...
        for donor in almanot_donors:
            if pd.notna(donor):
                self.assertIn(donor, donation_donors, f"Donor '{donor}' in almanot not found in donations")
        ...
        for _, almanot_row in almanot_df.iterrows():
            ...
            if pd.notna(donor):
                ...
                if not donor_donations.empty:
                    ...
                    self.assertLessEqual(monthly_amount, donation_amount * 2,
    ...
    def test_children_count_validation(self):
        ...
        for _, row in df.iterrows():
            ...
            self.assertLessEqual(children_count, 20, "Number of children should not be unreasonably high")
    ...
    def test_duplicate_validation(self):
        ...
        self.assertGreater(duplicate_combinations, 0, "Should have duplicate name-donor combinations")
...
class TestDataQualityMetrics(unittest.TestCase):
    ...
    def test_completeness_validation(self):
        ...
        for col in df.columns:
            ...
            self.assertGreaterEqual(completeness, 0.6, f"Column '{col}' completeness should be at least 60%")
    ...
    def test_consistency_validation(self):
        ...
        for col in df.columns:
            self.assertEqual(len(df[col]), len(df), f"Column '{col}' should have same length as DataFrame")
    ...
    def test_outlier_detection(self):
        ...
        test_data = {
            "סכום חודשי": [2000, 1500, 1000, 50000, 2000]  # One outlier
        }
        df = pd.DataFrame(test_data)
...
def run_data_validation_tests():
    ...
    # Print results
    print("=" * 60)
    print(f"📊 Data Validation Test Results: {result.testsRun - len(result.failures) - len(result.errors)}/{result.testsRun} tests passed")
