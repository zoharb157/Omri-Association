...
def create_network_editor(
    almanot_df: pd.DataFrame, donations_df: pd.DataFrame, investors_df: pd.DataFrame
):
...
def create_add_connection_tab(
    ...
    with st.form("add_connection_form"):
        ...
        with col1:
            ...
            # Show widow details
            if selected_widow:
                widow_data = widows_without_donors[
                    widows_without_donors["שם "] == selected_widow
                ].iloc[0]
                st.info(
        ...
        with col2:
            ...
            monthly_amount = st.number_input(
                "סכום תמיכה חודשי (₪)", min_value=0, value=1000, step=100, key="add_amount"
            )
        ...
        if st.form_submit_button("➕ הוסף קשר", use_container_width=True):
            if add_connection_to_sheets(selected_widow, selected_donor, monthly_amount, almanot_df):
                st.success(
                    f"✅ קשר נוסף בהצלחה: {selected_donor} → {selected_widow} (₪{monthly_amount:,.0f})"
                )
                st.rerun()
            else:
...
def create_edit_connection_tab(
    almanot_df: pd.DataFrame, donations_df: pd.DataFrame, investors_df: pd.DataFrame
):
    ...
    with st.form("edit_connection_form"):
        ...
        with col2:
            ...
            new_donor = st.selectbox(
                ...
                donor_options,
                key="edit_donor",
                index=donor_options.index(current_donor) if current_donor in donor_options else 0,
            )

            # New monthly support amount
            new_amount = st.number_input(
                "סכום חדש (₪)", min_value=0, value=int(current_amount), step=100, key="edit_amount"
            )
        ...
        if st.form_submit_button("✏️ עדכן קשר", use_container_width=True):
            if update_connection_in_sheets(selected_widow, new_donor, new_amount, almanot_df):
                st.success(
                    f"✅ קשר עודכן בהצלחה: {new_donor} → {selected_widow} (₪{new_amount:,.0f})"
                )
                st.rerun()
            else:
...
def create_delete_connection_tab(
    almanot_df: pd.DataFrame, donations_df: pd.DataFrame, investors_df: pd.DataFrame
):
    ...
    with st.form("delete_connection_form"):
        ...
        if st.form_submit_button(
            "🗑️ מחק קשר", use_container_width=True, disabled=not confirm_delete
        ):
...
def add_connection_to_sheets(
    widow_name: str, donor_name: str, monthly_amount: float, almanot_df: pd.DataFrame
) -> bool:
...
def update_connection_in_sheets(
    widow_name: str, new_donor: str, new_amount: float, almanot_df: pd.DataFrame
) -> bool:
