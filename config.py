...
class DashboardConfig:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        config = {
            ...
            'GOOGLE_SHEETS_SPREADSHEET_ID': os.getenv('GOOGLE_SHEETS_SPREADSHEET_ID', '1zo3Rnmmykvd55owzQyGPSjx6cYfy4SB3SZc-Ku7UcOo'),
    ...
    def validate(self) -> bool:
        ...
        try:
            ...
            if not os.path.exists(self.get('GOOGLE_SHEETS_SERVICE_ACCOUNT_FILE')):
                logging.warning(f"Service account file not found: {self.get('GOOGLE_SHEETS_SERVICE_ACCOUNT_FILE')}")
...
def show_config_ui():
    ...
    with col1:
        port = st.number_input("פורט", value=get_setting('DASHBOARD_PORT'), min_value=1, max_value=65535)
        host = st.text_input("כתובת", value=get_setting('DASHBOARD_HOST'))
        refresh_interval = st.number_input("מרווח רענון נתונים (שניות)", value=get_setting('DATA_REFRESH_INTERVAL'), min_value=30, max_value=3600)
    ...
    with col1:
        ...
        session_timeout = st.number_input("פסק זמן סשן (שניות)", value=get_setting('SESSION_TIMEOUT'), min_value=300, max_value=86400)
    
    with col2:
        enable_public = st.checkbox("אפשר גישה ציבורית", value=get_setting('ENABLE_PUBLIC_ACCESS'))
        max_requests = st.number_input("מקסימום בקשות במקביל", value=get_setting('MAX_CONCURRENT_REQUESTS'), min_value=1, max_value=100)
