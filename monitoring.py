...
class DashboardHealthChecker:
    ...
    def add_health_check(self, name: str, check_func, critical: bool = False):
        ...
        self.health_checks.append({"name": name, "function": check_func, "critical": critical})
...
def show_monitoring_dashboard():
    ...
    if trends and len(trends["cpu_percent"]) > 1:
        ...
        # Add CPU trend
        fig.add_trace(go.Scatter(y=trends["cpu_percent"], name="CPU %", line=dict(color="red")))

        # Add Memory trend
        fig.add_trace(
            go.Scatter(y=trends["memory_percent"], name="Memory %", line=dict(color="blue"))
        )

        # Add Disk trend
        fig.add_trace(go.Scatter(y=trends["disk_percent"], name="Disk %", line=dict(color="green")))

        fig.update_layout(
            title="מגמות ביצועים לאורך זמן", xaxis_title="מדידות", yaxis_title="אחוזים", height=400
        )
...
health_checker.add_health_check(
    "Google Sheets Connection", check_google_sheets_connection, critical=True
)
